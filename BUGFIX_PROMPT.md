...
const handleSave = async () => {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Optional: Helix Coach Nachricht
            // triggerMessage('journalSaved');
        } else {
...
**In `app/journal/page.tsx` oder `app/page.tsx`:**
```typescript
...
useEffect(() => {
    if (userId) {
        getMetricsHistory(userId, 7).then(metrics => {
            // Extrahiere z.B. energy_level Werte
    ...
    useEffect(() => {
        if (userId) {
            Promise.all([
                ...
                // Adherence berechnen: Check-ins der letzten 7 Tage / (Stack-Größe * 7)
                // TODO: Implementiere getCheckInsLast7Days() wenn nötig
                // Für jetzt: Placeholder
                setAdherence(84); // Placeholder
                setIsLoading(false);
            });
        }
    }, [userId]);
...
const getLevelStartXP = (level: number) => (level - 1) * 500;
const getLevelEndXP = (level: number) => level * 500;

// In XPDisplay:
const currentLevelStart = getLevelStartXP(level);
const currentLevelEnd = getLevelEndXP(level);
const progressInLevel = xp - currentLevelStart;
const progress = (progressInLevel / (currentLevelEnd - currentLevelStart)) * 100;
```

**Oder einfacher in `components/dashboard/XPDisplay.tsx`:**
```typescript
...
### Fix 14: Unused Components prüfen
- `components/ui/TiltCard.tsx` - Wird nirgends verwendet? Entweder verwenden oder löschen.
- `components/library/BioWheel.tsx` - Wird noch verwendet? Falls nicht, löschen (wurde durch Carousel3D ersetzt).
- `components/supplements/SupplementDrawer.tsx` - Wird noch verwendet? Falls nicht, löschen.
...
1. ✅ Journal speichert in Supabase
2. ✅ Check-In Status bleibt nach Refresh erhalten
3. ✅ ActivityGraph zeigt echte Daten
4. ✅ Disclaimer wird beim ersten Start angezeigt
5. ✅ Profile zeigt echte Statistiken
6. ✅ XP Progress-Bar ist korrekt
7. ✅ Supabase hat Error-Handling
8. ✅ CheckInButton hat besseres Icon
9. ✅ Stack hat Fortschrittsindikator
10. ✅ Code-Quality Issues behoben

---

## 📝 ZUSÄTZLICHE HINWEISE

- Die Library mit 3D-Karussell ist bereits gut implementiert (`Carousel3D`, `SupplementCard3D`)
- `getUserStackIds()` existiert bereits in `supabaseService.ts`
- `DosageModal` existiert bereits
- Supplements haben bereits `emoji` Felder

**Wichtig:** Teste nach jedem Fix die Funktionalität!
